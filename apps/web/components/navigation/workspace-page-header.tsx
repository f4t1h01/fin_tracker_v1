...
export function WorkspacePageHeader({ eyebrow, title, description, actions, className }: WorkspacePageHeaderProps) {
  return (
    <header className={cn("soft-rise mb-8 grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4", className)}>
      <div className="min-w-0 space-y-4">
        ...
        {actions?.length ? <WorkspaceHeaderActions groups={actions} /> : null}
      </div>
