...
export async function extractVoiceTransactionDraft(params: {
  ...
  const prompt = [
    ...
    "If the transcript contains multiple expenses that share the same kind, currency, and best visible category, add their amounts together and return one EXPENSE draft.",
    "When summing multiple expenses, choose the most specific visible category that fits all items; if different children share a visible parent category, use that parent category.",
    "When summing multiple expenses, write a short note that summarizes the items and add a warning that multiple expenses were combined.",
    "If multiple items have different currencies, mix income and expense, or cannot reasonably share one visible category, set warnings to explain that the user should split the recording and leave uncertain core fields null.",
