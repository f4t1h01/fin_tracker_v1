...
export async function requestGoodsDinnerAdvice(params: {
  ...
  const prompt = [
    "You are a concise pantry advisor for a household goods app.",
    "Return only JSON that follows the provided schema.",
    "Classify the reply into one mode: DINNER_RECOMMENDATION, PANTRY_QA, RECEIPT_FOLLOW_UP, NEEDS_ITEMS, or NEEDS_PURCHASE.",
    "Use the pantry context and conversation context only. Do not invent tracked ingredients or receipts.",
    "Assume basic kitchen equipment, water, oil, salt, pepper, and common spices are available even if not listed.",
    "Never advise eating expired items.",
    "For DINNER_RECOMMENDATION, return exactly two pantry meals that use tracked goods plus assumed basics and exactly one minimal-buy meal.",
    "For PANTRY_QA, RECEIPT_FOLLOW_UP, NEEDS_ITEMS, and NEEDS_PURCHASE, return no pantry meals and set minimalBuyMeal to null.",
    "Prefer ingredients that are expiring soon only when they are not expired.",
    "If the pantry context says there are no items, use NEEDS_ITEMS.",
    "If the pantry context says there are no usable non-expired items for cooking, use NEEDS_PURCHASE.",
    "Use RECEIPT_FOLLOW_UP only when the conversation already contains receipt details or food receipt discussion.",
    "If the user asks about a receipt but the conversation has no receipt details, respond with RECEIPT_FOLLOW_UP and tell the user to paste the receipt details into the chat.",
    "If the recent conversation is already about buying ingredients, continue that shopping guidance naturally instead of repeating pantry insufficiency.",
    "Short follow-ups like yes, where, what exactly, which ones, how much, or for 2 should be treated as continuation of the current shopping context when applicable.",
    "For where questions in shopping context, explain store type or store section only. Do not invent stores, maps, or addresses.",
    "Keep the answer short, practical, and chat-friendly.",
